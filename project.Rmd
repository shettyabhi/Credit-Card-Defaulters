...
To find the correlation between the probability of default committed by credit card users with its associated factors which have been extracted from the database of an important bank in Taiwan with data of 6 months.
...
The raw data is obtained from UCI Machine Learning Repository. The data set contains 23 variables out of which  I’ve identified 6 main factors to run our analysis to predict the credibility of potential credit card customers.
...
Following the traditional steps of data analysis, I started off with cleaning and filtering the data to get rid of unwanted noise or outliers and null values. To do so, I first studied each variable, calculated its statistics like mean, standard deviation and range, and plotted its distribution. I used the log() function in R in order to normalize our variables. I then categorized the data using different combinations of the variables to find correlation between the target variable- defaulters and rest of the variables. This not only gave an insight of the dataset but also helped in forming a basis for the hypothesis tests to be carried out on the dataset. Based on this, I came up with three hypothesis and carried out Test of Hypothesis. I also calculated confidence intervals.

In addition to covering concepts learned in the lecture/lab, I utilized the 'rcompanion' package to plot the normal histogram for 'limit_bal' variable, 'corrplot' pakcage to plot the correlation plot of the dataset, 'rpart', 'rpart.plot', 'party' packages for plotting the decision tree.
...
I carried out basic statistical analysis of the 'limit_bal' variable to find their measure of central tendency, variablity and symmetry as given below:  
...
setwd("C:/Users/abhis/OneDrive/Desktop/")
...
I selected the required columns and categorized the data with different combinations. Then ,created a new table with total people in each category with their respective number of defaulters along with their probabilities as shown below:
...
I then visualized all the data created and all the target variables.
...
I performed the goodness of fit test on the 'limit_bal' variable and plotted the density, cdf, qq and pp plots. The four graphs which are plotted are used to test the goodness of the distribution like how fair the dataset is fitted. Refer to the results below:
...
I tested for three different hypothesis stated as follows:

#### Hypothesis 1

I are going to test if unmarried men with higher level of education default more.
...
Since n is greater than 30, I applied z test.

I consider 95% confidence interval. Hence, its z value is 1.96
...
The z score does not fall in the rejection region, hence I fail to reject the null hypothesis. So I conclude that unmarried men with higher education level default more.


#### Hypothesis 2

I are going to compare the limit balances given for male and female customers and test for who is given a greater limit balance. I shall use two sample left tailed test.
...
The z score does not fall in the rejection region, hence I fail to reject the null hypothesis. So I conclude that male customers are given greater limit balance as compared to female customers.


### Constructing confidence interval

I computed a 95% confidence interval for the difference between the proportions of male defaulters who are married and have higher level of education from two samples.

I first calculated the error and then found the lower and upper limits of the confidence interval.
...
In our analysis, I have used classification trees to identify the trend of the credit card defaulters based on their alloted limit balance, age, sex and marital status.
